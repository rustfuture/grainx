...
fn test_prediction_simple_average() {
    ...
    assert!(
        (prediction - 40.0).abs() < 0.001,
        "Prediction should be 40.0"
    );
}
...
fn test_metric_formula_complex() {
    ...
    assert!(
        (result - 100.0).abs() < 0.001,
        "60 * 1.5 + 10 should be 100"
    );
}
...
fn test_performance_monitor_adaptive_refresh() {
    ...
    assert!(
        high_cpu_refresh >= 1000,
        "High CPU should result in slower refresh"
    );
    ...
    assert!(
        low_cpu_refresh <= 500,
        "Low CPU should result in faster refresh"
    );
}
...
fn test_performance_monitor_frame_skipping() {
    ...
    assert!(
        perf.should_skip_frame(98.0),
        "Very high CPU should skip frames"
    );
    assert!(
        !perf.should_skip_frame(50.0),
        "Normal CPU should not skip frames"
    );
}
