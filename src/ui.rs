...
pub async fn draw_dashboard(
    ...
    net_rx_points: &mut Vec<(f64, f64)>,
    net_tx_points: &mut Vec<(f64, f64)>,
    cpu_history: &mut Vec<f64>,
    mem_history: &mut Vec<f64>,
    last_rx_bytes: &mut u64,
    last_tx_bytes: &mut u64,
    ...
    current_net_rx_y: &mut f64,
    current_net_tx_y: &mut f64,
    ...
    let alerts = backend.take_alerts();
    ...
    *iteration_count += 1;
    ...
    if let Some(anomaly) = anomaly_detector
    ...
    *current_mem_y_val = *current_mem_y_val * 0.8 + target_mem_y * 0.2;
    ...
    let rx_kbps = throughput_kbps(rx_bytes, *last_rx_bytes);
    let tx_kbps = throughput_kbps(tx_bytes, *last_tx_bytes);
    *last_rx_bytes = rx_bytes;
    *last_tx_bytes = tx_bytes;

    canvas.set_cursor(0, layout.net_rect.y.saturating_sub(1))?;
    canvas.set_color(palette.label)?;
    canvas.draw_str(&format!(
    ...
    let target_net_rx_y = (rx_kbps / max_net_kbps) * layout.net_rect.height as f64;
    let target_net_tx_y = (tx_kbps / max_net_kbps) * layout.net_rect.height as f64;
    *current_net_rx_y = *current_net_rx_y * 0.8 + target_net_rx_y * 0.2;
    *current_net_tx_y = *current_net_tx_y * 0.8 + target_net_tx_y * 0.2;

    net_rx_points.push((*iteration_count as f64, *current_net_rx_y));
    net_tx_points.push((*iteration_count as f64, *current_net_tx_y));
    if net_rx_points.len() > layout.net_rect.width as usize {
        net_rx_points.remove(0);
        net_tx_points.remove(0);
        for p in net_rx_points.iter_mut() {
            p.0 -= 1.0;
        }
        for p in net_tx_points.iter_mut() {
            p.0 -= 1.0;
        }
    }

    for y in layout.net_rect.y..(layout.net_rect.y + layout.net_rect.height) {
        canvas.set_cursor(layout.net_rect.x, y)?;
        canvas.draw_str(&" ".repeat(layout.net_rect.width as usize))?;
    }

    canvas.set_color(palette.accent)?;
    canvas.draw_braille_line(net_rx_points, &layout.net_rect)?;
    canvas.set_color(palette.warning)?;
    canvas.draw_braille_line(net_tx_points, &layout.net_rect)?;
    ...
    if dashboard_config.show_correlations || dashboard_config.show_predictions {
        ...
        if dashboard_config.show_correlations
            && *iteration_count % 10 == 0
            && cpu_history.len() > 5
            && mem_history.len() > 5
            && let Some(correlation) = calculate_correlation(cpu_history, mem_history)
        {
            canvas.set_cursor(0, layout.network_start_y + 4)?;
        ...
        if dashboard_config.show_predictions
            && let Some(predicted_cpu) = predict_next_value(cpu_history, 5)
        {
    ...
    for (i, (pid, name, cpu, memory)) in processes
        .iter()
        .take(dashboard_config.max_processes)
        .enumerate()
    {
